...
class appFunctions():
    ...
    @staticmethod
    def main(dbFolder):
        createUserTable = """CREATE TABLE IF NOT EXISTS Students (
                                COURSE_CODE TEXT PRIMARY KEY,
                                COURSE TEXT
                            );"""
        createUserTable1 = """CREATE TABLE IF NOT EXISTS Students1 (
                                ID TEXT PRIMARY KEY,
                                FULL_NAME TEXT,
                                GENDER TEXT,
                                YEAR_LEVEL TEXT,
    ...
    def inputCourse(self, dbFolder):
        ...
        insert_course_data_sql = "INSERT INTO Students (COURSE, COURSE_CODE) VALUES (?, ?)"

        try:
            c = conn.cursor()

            # Check if the course code already exists
            select_course_sql = "SELECT * FROM Students WHERE COURSE_CODE = ?"
    ...
    def inputCourse1(self, dbFolder):
        ...
        # Check if any required field is empty
        if not idNum or not fullName or not yearLevel or not courseCode:
            QMessageBox.warning(self, "Missing Fields", "Please fill in all required fields.")
            return
    ...
    def displayCourse(self, rows):
        ...
        course_codes = [row[1] for row in rows]  # Fetch COURSE_CODE data
    ...
    def searchData1(self, dbFolder):
        ...
        def perform_search():
            ...
            rows = appFunctions.searchUsers1(dbFolder, search_terms)
    ...
    @staticmethod
    def searchUsers1(dbFolder, search_terms):
        conn = appFunctions.createConnection(dbFolder)
        _searchUsers = "SELECT * FROM Students1 WHERE ID LIKE ? OR FULL_NAME LIKE ? OR GENDER LIKE ? OR YEAR_LEVEL LIKE ? OR COURSE_CODE LIKE ?"
        try:
            c = conn.cursor()
            ...
            if len(search_terms) == 0:
                c.execute(_searchUsers, ('%', '%', '%', '%', '%'))
            elif len(search_terms) == 1:
                c.execute(_searchUsers, (params[0], params[0], params[0], params[0], params[0]))
            elif len(search_terms) == 2:
                c.execute(_searchUsers, (params[0], params[1], params[0], params[1], params[1]))
            elif len(search_terms) == 3:
                c.execute(_searchUsers, (params[0], params[1], params[2], params[0], params[2]))
            elif len(search_terms) == 4:
                c.execute(_searchUsers, (params[0], params[1], params[2], params[3], params[3]))
            else:
                c.execute(_searchUsers, tuple(params + params[:5]))  # Use the first 5 search terms
    ...
    def searchCourses(dbFolder, search_terms):
        conn = appFunctions.createConnection(dbFolder)
        _searchCourses = "SELECT * FROM Students WHERE COURSE_CODE LIKE ? OR COURSE LIKE ?"
        try:
            c = conn.cursor()
            params = ['%{}%'.format(term) for term in search_terms]
    ...
    def deleteStudent(self):
        ...
        delete_student_sql = "DELETE FROM Students1 WHERE ID = ?"
    ...
    def deleteCourse(self):
        ...
        delete_course_sql = "DELETE FROM Students WHERE COURSE = ?"
        delete_student_sql = "DELETE FROM Students1 WHERE COURSE_CODE = ?"

        try:
            c = conn.cursor()

            # Check if the course exists
            select_course_sql = "SELECT * FROM Students WHERE COURSE = ?"
            c.execute(select_course_sql, (course_code,))
            existing_course = c.fetchone()
    ...
    def updateStudent(self):
        ...
        if ok:
            ...
            conn = appFunctions.createConnection(dbFolder)
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM Students1 WHERE ID=?", (id_number,))
            ...
            if student:
                ...
                # Get the course codes from the Students table
                course_codes = appFunctions.getAllUsers(dbFolder)
    ...
    def updateStudentData(self):
        ...
        conn = appFunctions.createConnection(dbFolder)
        cursor = conn.cursor()
        cursor.execute("UPDATE Students1 SET FULL_NAME=?, GENDER=?, YEAR_LEVEL=?, COURSE_CODE=? WHERE ID=?",
    ...
    def updateCourseData(self):
        ...
        conn = appFunctions.createConnection(dbFolder)
        cursor = conn.cursor()
        cursor.execute("UPDATE Students SET COURSE_CODE =? WHERE COURSE = ?",
                    (course, course_code))
        conn.commit()
