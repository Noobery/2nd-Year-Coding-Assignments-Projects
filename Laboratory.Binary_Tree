...
// Start with the root node
    ...
    while (curr != nullptr) {
        ...
        else if (val > curr->element) {
            curr = curr->right;
        }
        // If the value is equal to the current node's value, return 1
        else {
            return 1;
        }
    }

    // If we reach a leaf node and haven't returned 1, the value is not in the tree
    return 0;
}
...
template <class T> void BSTree<T>::insert(T &val) {
    ...
    BinNode<T> *newNode = makeBinNode(val);
    // Check if the tree is empty
    if (root == NULL) {
        // If the tree is empty, set the new node as the root
        root = newNode;
    } else {
        // If the tree is not empty, traverse the tree to find the correct position for the new node
        BinNode<T> *p = root;
        while (p != NULL) {
            // Check if the new value is less than or equal to the current node's value
            if (val <= p->element) {
                // If the new value is less than or equal to the current node's value, try to insert it in the left subtree
                if (p->left == NULL) {
                ...
                // If the left child is not empty, go to the left child and try again
                p = p->left;
            } else {
                // If the new value is greater than the current node's value, try to insert it in the right subtree
                if (p->right == NULL) {
                ...
                // If the right child is not empty, go to the right child and try again
                p = p->right;
            }
        }
    }
...
// Check if the tree is empty
    ...
    while (currentNode != NULL) {
        ...
        if (val < currentNode->element) {
            ...
            // Go right if the value is greater than the current node's value
    ...
    if (currentNode == NULL) {
        ...
        // Return default value if the node was not found
        return T();
    }
    ...
    if (parentNode == NULL) {
        ...
        // Node is the root, remove the root
        root = removeTop(currentNode);

    } else if (val < parentNode->element) {

        // Node is the left child, remove it
        parentNode->left = removeTop(currentNode);

    } else {

        // Node is the right child, remove it
